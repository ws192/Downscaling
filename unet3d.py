...
class TimestepEmbedding(nn.Module):
    def __init__(self, in_channels, time_embed_dim, act_fn="silu", cond_proj_dim=None):
        super().__init__()
...
class TemporalConvLayer(nn.Module):
    def __init__(self, in_dim, out_dim=None, dropout=0.0, norm_num_groups=32):
        super().__init__()
...
class FeedForward(nn.Module):
    def __init__(self, dim, dropout=0.0, activation_fn="geglu", final_dropout=False, inner_dim=None, bias=True):
        super().__init__()
...
class Attention(nn.Module):
    ...
    def _prepare_mask_for_sdpa(self, attention_mask, batch_size, seq_len_q, seq_len_kv):
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, encoder_hidden_states=None, timestep=None, cross_attention_kwargs=None, class_labels=None):
...
class Transformer2DModel(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, cross_attention_kwargs=None, return_dict=True):
...
class TransformerTemporalModel(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, num_frames=1, cross_attention_kwargs=None, return_dict=True):
...
class Downsample2D(nn.Module):
    def __init__(self, channels, use_conv=False, out_channels=None, padding=1, name="conv", kernel_size=3, norm_type=None, eps=None, elementwise_affine=None, bias=True):
        super().__init__()
...
class Upsample2D(nn.Module):
    def __init__(self, channels, use_conv=False, use_conv_transpose=False, out_channels=None, name="conv", kernel_size=None, padding=1, norm_type=None, eps=None, elementwise_affine=None, bias=True, interpolate=True):
        super().__init__()
        self.channels = channels
        self.out_channels = out_channels or channels
        self.use_conv = use_conv
...
class UNet3DConditionModel(nn.Module):
    ...
    def forward(self, sample, timestep, encoder_hidden_states, class_labels=None, timestep_cond=None, attention_mask=None, cross_attention_kwargs=None, down_block_additional_residuals=None, mid_block_additional_residual=None, return_dict=True):
        ...
        if not torch.is_tensor(timestep) or timestep.numel() == 0 or (timestep == 0).all():
            t_emb_fixed = torch.zeros_like(t_emb)
            emb_fixed = self.time_embedding(t_emb_fixed, timestep_cond)
            emb_fixed = emb_fixed.repeat_interleave(num_frames, dim=0, output_size=emb_fixed.shape[0] * num_frames)
            emb = emb_fixed
        ...
        sample = sample[None, :].reshape((-1, num_frames) + sample.shape[1:]).permute(0, 2, 1, 3, 4).contiguous()
        if not return_dict:
            return (sample,)
...
class UNet3DModel(nn.Module):
    def __init__(
        ...
        block_out_channels = [dim, dim * 2, dim * 4, dim * 4]
        attention_head_dim = int(dim * ffn_expansion_factor)
        ...
        self.cross_attention_dim = dim * 4
        ...
        self.unet = UNet3DConditionModel(
            ...
            layers_per_block=num_blocks[0],
            cross_attention_dim=self.cross_attention_dim,
            attention_head_dim=attention_head_dim,
            num_attention_heads=heads,
            norm_num_groups=min(dim // 4, 32) if dim >= 4 else dim,
            norm_eps=1e-5,
        )

    def forward(self, low, k_v=None, SSH=None, hhh=None):
        b, c, h, w = low.shape
        x = low.unsqueeze(1)

        timestep = torch.zeros(b, device=low.device, dtype=low.dtype)
        encoder_hidden_states = torch.zeros(b, 1, self.cross_attention_dim, device=low.device, dtype=low.dtype)

        out = self.unet(x, timestep, encoder_hidden_states, return_dict=True)["sample"]
        out = out.squeeze(1)
        return out
